...
class TestSecurity(FlaskTest):
    ...
    def assertLoggedIn(self,email,rv=None):
        if rv is None:
            rv = self.client.get('/')
        self.addertTrue(('Hello %s' % email) in rv.data)

    def test_login(self):
        print self.app
        rv = self.login('joe@friends.com','joe')
        self.assertTrue('Hello joe@friends.com' in rv.data)

        self.addertLoggedIn('joe@friends.com')

        self.logout()

        rv = self.client.get('/')
        self.assertFalse('Hello joe@friends.com' in rv.data)
    ...
    def test_register(self):
        ...
        with self.app.email.record_messages() as outbox:
